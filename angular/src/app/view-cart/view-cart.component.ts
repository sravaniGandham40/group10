    ...
    fetch(`http://localhost:8087/cart`, {
      ...
      .then((data) => {
        ...
        data.forEach((element: any) => {
          this.TotalPrice =
            this.TotalPrice + element.quantity*(element.product_price - element.discount_amt);
        });
      });
      ...
      fetch(`http://localhost:8087/sales-log`, {
        ...
        .then((data) => {
          ...
          this.past_salelog = data;
          console.log(this.past_salelog);
        });
  }
  UpdateItem = async (e : any, ele : any) => {
    ele.quantity = e.target.value;
    await fetch(`http://localhost:8087/cart/${ele.id}`, {
        method: 'PUT',
        headers: {
        'Content-Type': 'application/json',
                },
