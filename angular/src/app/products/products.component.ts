    ...
    await fetch(`http://localhost:8087/cart`, {
      ...
      .then((cart_data) => {
        ...
        cart_data = cart_data.filter((item: any) => {
          return item.username == localStorage.getItem('user') && item.product_id == data.product_id;
        });
        
        //console.log(cart_data);
        if (cart_data.length){
        cart_data[0].quantity = data.quantity + parseInt(cart_data[0].quantity);
        fetch(`http://localhost:8087/cart/${cart_data[0].product_id}`, {
        method: 'PUT',
        headers: {
        'Content-Type': 'application/json',
                },
                body: JSON.stringify(cart_data[0]),
           });
           this.added = true;
          ...
          let res = fetch('http://localhost:8087/cart', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
